...
fastqc_data="/home/rstudio/data/mydatalocal/data/fastqc_data_test_trim"
mkdir -p $fastqc_data
cd $fastqc_data
...
Time=$(((end - start)/60))
echo "$file $Time" >> $time_files/time_fastqc_test_trim.txt
done

#Regroup quality controls (multiqc): 

##Create a variable with the data set and the time files directory: 
data_2="/home/rstudio/data/mydatalocal/data/fastqc_data_test_trim"
time_files="/home/rstudio/data/mydatalocal/data/time_files"
...
Time=$(((end - start)/60))
echo "$file $Time" >> $time_files/time_multiqc_test_trim.txt
done